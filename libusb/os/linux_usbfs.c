...
static int _get_usbfs_fd(struct libusb_device *dev, mode_t mode, int silent)
{
	...
	if (priv->cache_fd != -1)
	{
		...
		if (fd != -1) {
			fcntl(fd, F_SETFD, fcntl(fd, F_GETFD) | (supports_flag_cloexec ? FD_CLOEXEC : 0));
			lseek(fd, 0, SEEK_SET);
		}
	}
	else
 ...
 * is running as root. Due to the fact that on Android file descriptor is provided via JINI 
 * from Android's USB related Java APIs open() API is not used and we do not really need 
 * healthy /dev/bus/usb. Just assume it exists to avoid libusb_init failure. */
#ifdef __ANDROID__
	if (ret == NULL)
		ret = "/dev/bus/usb";
#endif
...
static int op_init(struct libusb_context *ctx)
{
	...
	if (sysfs_can_relate_devices || sysfs_has_descriptors) {
		...
		if (r != 0 || !S_ISDIR(statbuf.st_mode)) {
			...
			DIR *dir = opendir(SYSFS_DEVICE_PATH);
			if (!dir) {
				usbi_warn(ctx, "sysfs mounted but not accessible (permission?)");
				sysfs_can_relate_devices = 0;
				sysfs_has_descriptors = 0;
			}
			else
...
static int op_open(struct libusb_device_handle *handle)
{
	...
	if (handle->associated_fd != -1) {
		...
		fcntl(hpriv->fd, F_SETFD, fcntl(hpriv->fd, F_GETFD) | (supports_flag_cloexec ? FD_CLOEXEC : 0));
